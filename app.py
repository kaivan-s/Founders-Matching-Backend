...
@app.route('/api/founders/tutorial-status', methods=['GET'])
def get_tutorial_status():
    """Get discovery tutorial completion status"""
    try:
        clerk_user_id = get_clerk_user_id()
        if not clerk_user_id:
            return jsonify({"error": "User ID required"}), 401
        ...
        supabase = get_supabase()
        result = supabase.table('founders').select('discovery_tutorial_completed').eq('clerk_user_id', clerk_user_id).execute()
        
        if result.data:
            # Check if column exists, default to False if not
            tutorial_completed = result.data[0].get('discovery_tutorial_completed', False)
            return jsonify({
                'tutorial_completed': bool(tutorial_completed)
            }), 200
        else:
            # Founder doesn't exist yet, tutorial not completed
            return jsonify({
                'tutorial_completed': False
            }), 200
            
    except Exception as e:
        # If column doesn't exist yet, return False (tutorial not completed)
        log_warning(f"Error getting tutorial status (column may not exist): {e}")
        return jsonify({
            'tutorial_completed': False
        }), 200

@app.route('/api/founders/complete-tutorial', methods=['POST'])
@limiter.limit(RATE_LIMITS['moderate'])
def complete_tutorial():
    """Mark discovery tutorial as completed"""
    try:
        clerk_user_id = get_clerk_user_id()
        if not clerk_user_id:
            return jsonify({"error": "User ID required"}), 401
        ...
        # Use a try-except to handle case where column doesn't exist yet
        try:
            supabase.table('founders').update({
                'discovery_tutorial_completed': True
            ...
            # Column might not exist - log warning but don't fail
            log_warning(f"Could not update tutorial status (column may not exist): {e}")
            return jsonify({
                'success': True,
                'message': 'Tutorial completion recorded (column may need migration)'
            }), 200
            
    except Exception as e:
        log_error("Error completing tutorial", error=e)
        return jsonify({"error": str(e)}), 500
